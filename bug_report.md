...
## Bug 16: Room Stats Race Condition

* **File(s) / Line(s):** `app/services/stats.py`, lines 15-30
* **What the bug was and why it caused incorrect behavior:**
  In `stats.py`, the functions `record_create` and `record_cancel` retrieved current room counts and revenue (`current = _stats.get(...)`), invoked an artificial sleep (`_aggregate_pause()`, `time.sleep(0.1)`), and then incremented/decremented and reassigned the dictionary (`_stats[room_id] = ...`). Under concurrent booking creations or cancellations, multiple threads simultaneously read the exact same un-updated count and revenue totals before any thread woke up from `_aggregate_pause()` to write back its changes. As a result, concurrent booking activity overwrote intermediate increments, leading to dropped booking counts and missing revenue in `GET /rooms/{id}/stats`. This violated Business Rule 14 (`question.md` Section 3), which requires live statistics to always be consistent with confirmed bookings, including after bursts of concurrent activity.
* **How it was fixed:**
  Imported `threading`, instantiated a module-level mutex `_lock = threading.Lock()`, and wrapped the bodies of `record_create`, `record_cancel`, and `get` inside `with _lock:` blocks (`app/services/stats.py`) to guarantee atomic updates and thread-safe reads across all room statistical tracking.

## Bug 17: Financial Rounding & Truncation Mismatch

* **File(s) / Line(s):** `app/services/refunds.py`, lines 15-17
* **What the bug was and why it caused incorrect behavior:**
